...
import { authorizeGatewayConnect, isLocalDirectRequest, safeEqual } from "../../auth.js";
  ...
  socket.on("message", async (data) => {
    ...
    try {
      ...
      if (!client) {
        ...
        // In approval mode, valid device identity is sufficient to enter pairing flow.
        // Already-paired devices passed via device-token above. New devices proceed
        // to the pairing check below where they enter pending state.
        // Ban check is inside approval bypass only — early ban would block already-paired
        // devices reconnecting from a banned IP, which is wrong.
