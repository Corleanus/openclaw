...
import { createPairBanManager, type PairBanManager } from "./pair-ban.js";
import { createPairRateLimiter } from "./pair-rate-limit.js";
...
export async function createGatewayRuntimeState(params: {
  ...
  // Create singleton ban manager + rate limiter at gateway runtime level.
  // Shared across all HTTP servers (IPv4 + IPv6) and the WS handler.
  const approvalConfig = params.resolvedAuth.approval;
  ...
  const pruneTimer = setInterval(() => pairRateLimiter.prune(), 60_000);
  pruneTimer.unref();
