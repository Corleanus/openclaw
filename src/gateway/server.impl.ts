...
export async function startGatewayServer(
  ...
  attachGatewayWsHandlers({
    ...
    extraHandlers: {
      ...
      "gateway.security.bans.list": async ({ respond }) => {
        const records = pairBanManager.listBanned();
        respond(true, { records }, undefined);
      },
      "gateway.security.bans.unban": async ({ params, respond }) => {
        const ip = typeof params.ip === "string" ? params.ip.trim() : "";
        if (!ip) {
          respond(false, undefined, { code: "INVALID_REQUEST", message: "ip is required" });
          return;
        }
        const ok = pairBanManager.unban(ip);
