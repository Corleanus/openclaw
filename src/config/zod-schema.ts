  ...
  .object({
    ...
    gateway: z
      .object({
        ...
        auth: z
          .object({
            mode: z.union([z.literal("token"), z.literal("password"), z.literal("approval")]).optional(),
            token: z.string().optional(),
            password: z.string().optional(),
            allowTailscale: z.boolean().optional(),
            approval: z
              .object({
                password: z.string().regex(/^[\x20-\x7E]+$/, "Password must be ASCII printable characters").min(1).optional(),
                maxAuthFailures: z.number().int().min(1).max(10).optional(),
                maxPendingPerIp: z.number().int().positive().optional(),
                cooldownMs: z.number().int().positive().optional(),
                rateLimitCooldownMs: z.number().int().positive().optional(),
                maxPendingTotal: z.number().int().positive().optional(),
                windowMs: z.number().int().min(1000).optional(),
                maxRequestsPerWindow: z.number().int().min(1).optional(),
              })
              .strict()
              .transform((val) => {
                if (val.rateLimitCooldownMs !== undefined && val.cooldownMs === undefined) {
                  return { ...val, cooldownMs: val.rateLimitCooldownMs };
                }
                return val;
              })
              .optional(),
