...
async function resolveCliOutput(params: {
  ...
  // Try known command-specific output first
  let fileOutput: string | null = null;
  if (commandId === "whisper-cli") {
    fileOutput = resolveWhisperCppOutputPath(params.args);
  } else if (commandId === "whisper") {
    fileOutput = resolveWhisperOutputPath(params.args, params.mediaPath);
  }
  // Fallback: try arg-based detection regardless of command name.
  // Handles wrapper scripts (e.g., whisper-wrapper.cmd) that pass through to whisper/whisper-cli.
  if (!fileOutput) {
    fileOutput =
