...
async function runGatewayCommand(opts: GatewayRunOpts) {
  ...
  // Capture user-set env port before startGatewayServer stamps process.env.OPENCLAW_GATEWAY_PORT.
  // This preserves the user's original env override across in-process SIGUSR1 restarts.
  const envPortRaw =
    process.env.OPENCLAW_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  const envPortOverride = (() => {
    if (!envPortRaw) return null;
  ...
  const hasApprovalMode = resolvedAuthMode === "approval";
  if (bind !== "loopback" && !hasSharedSecret && !hasApprovalMode) {
    defaultRuntime.error(
      [
  ...
  try {
    await runGatewayLoop({
      ...
      start: async () => {
        // Re-resolve port on each restart iteration so config changes take effect.
        // CLI --port > user env var > fresh config > default.
        // Pass {} as env to bypass the env var startGatewayServer stamps on each start.
        const currentPort = portOverride ?? envPortOverride ?? resolveGatewayPort(loadConfig(), {});
        return await startGatewayServer(currentPort, {
          bind,
          auth:
