...
export async function runDaemonInstall(opts: DaemonInstallOptions) {
  ...
  // Config port is authoritative for install — env is a runtime override, not a persisted one.
  const configPort = cfg?.gateway?.port;
  const port = portOverride ?? (typeof configPort === "number" && configPort > 0 ? configPort : resolveGatewayPort(cfg));
