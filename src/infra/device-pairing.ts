...
export type DevicePairingResolution = {
  requestId: string;
  deviceId: string;
...
const RESOLUTION_TTL_MS = 24 * 60 * 60 * 1000; // 24 hours
...
function resolvePaths(baseDir?: string) {
  ...
  return {
    ...
    resolvedPath: path.join(dir, "resolved.json"),
  };
}
...
async function loadResolutions(baseDir?: string): Promise<Record<string, DevicePairingResolution>> {
  const { resolvedPath } = resolvePaths(baseDir);
  const raw = await readJSON<Record<string, DevicePairingResolution>>(resolvedPath);
  if (!raw) return {};
  const now = Date.now();
  const pruned: Record<string, DevicePairingResolution> = {};
  let changed = false;
  for (const [key, entry] of Object.entries(raw)) {
    if (now - entry.resolvedAtMs <= RESOLUTION_TTL_MS) {
      pruned[key] = entry;
    } else {
      changed = true;
    }
  ...
  if (changed) {
    const { resolvedPath: rp } = resolvePaths(baseDir);
    await writeJSONAtomic(rp, pruned);
  }
  return pruned;
}

async function saveResolution(resolution: DevicePairingResolution, baseDir?: string): Promise<void> {
  const resolutions = await loadResolutions(baseDir);
  resolutions[resolution.requestId] = resolution;
  const { resolvedPath } = resolvePaths(baseDir);
  await writeJSONAtomic(resolvedPath, resolutions);
}
...
export async function getDevicePairingResolution(
  requestId: string,
  baseDir?: string,
): Promise<DevicePairingResolution | null> {
  return await withLock(async () => {
    const resolutions = await loadResolutions(baseDir);
    return resolutions[requestId] ?? null;
  });
}
