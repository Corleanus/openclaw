...
export function recomputeNextRuns(state: CronServiceState) {
  ...
  for (const job of state.store.jobs) {
    ...
    if (job.catchUp && job.enabled && job.schedule.kind === "cron" && typeof job.state.nextRunAtMs === "number") {
      const lastRun = job.state.lastRunAtMs;
      if (lastRun === undefined) continue; // never ran → not a missed run, skip catch-up
      const prevScheduled = computePreviousRunAtMs(job.schedule, now);
      if (prevScheduled !== undefined && prevScheduled > lastRun) {
        state.deps.log.warn({ jobId: job.id, missedAt: prevScheduled, lastRun }, "cron: catch-up — firing missed run");
        job.state.nextRunAtMs = prevScheduled;
      }
    }
  }
}
...
export function createJob(state: CronServiceState, input: CronJobCreate): CronJob {
  ...
  const job: CronJob = {
    ...
    enabled,
    deleteAfterRun,
    catchUp: typeof input.catchUp === "boolean" ? input.catchUp : undefined,
    createdAtMs: now,
    updatedAtMs: now,
...
export function applyJobPatch(job: CronJob, patch: CronJobPatch) {
  ...
  if (typeof patch.catchUp === "boolean") {
