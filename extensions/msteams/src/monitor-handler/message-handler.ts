...
export function createMSTeamsMessageHandler(deps: MSTeamsMessageHandlerDeps) {
  ...
  const handleTeamsMessageNow = async (params: MSTeamsDebounceEntry) => {
    ...
    const dormancyResult = core.channel.routing.applyDormancyGate({
      ...
      messageTimestamp: parseMSTeamsActivityTimestamp(activity.timestamp)?.getTime(),
    });
    if (!dormancyResult.shouldProcess) {
      logVerboseMessage(`Skipping message: agent ${route.agentId} dormancy gate (${dormancyResult.reason})`);
      return;
    }
