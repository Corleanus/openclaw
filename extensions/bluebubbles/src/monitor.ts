...
async function processMessage(
  ...
  if (!dormancyResult.shouldProcess) {
    logVerbose(core, runtime, `Skipping message: agent ${route.agentId} dormancy gate (${dormancyResult.reason})`);
    return;
  }
...
async function processReaction(
  ...
  if (!dormancyResult.shouldProcess) {
    logVerbose(core, runtime, `Skipping reaction: agent ${route.agentId} dormancy gate (${dormancyResult.reason})`);
    return;
  }
